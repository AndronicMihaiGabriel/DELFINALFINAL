...
def login_web():
    try:
        if request.method == 'POST':
            ...
            cursor = conn.cursor()
            cursor.execute(
                'SELECT * FROM userweb WHERE username = ?', (username,))
...
def register_web():
    if request.method == 'POST':
        ...
        cursor = conn.cursor()
        cursor.execute(
            'SELECT id FROM userweb WHERE username = ?', (username,))
        if cursor.fetchone():
            cursor.close()
            conn.close()
            return render_template('register.html', error="Username deja există!")

        cursor.execute('SELECT id FROM userweb WHERE email = ?', (email,))
        if cursor.fetchone():
            cursor.close()
            conn.close()
        ...
        cursor.execute('INSERT INTO userweb (username, password, rol, email) VALUES (?, ?, ?, ?)',
...
def load_user(user_id):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute('SELECT * FROM userweb WHERE id = ?', (user_id,))
    user = cursor.fetchone()
    user = row_to_dict(cursor, user)
    cursor.close()
    conn.close()
...
def api_userweb():
    ...
    if username:
        cursor.execute(
            "SELECT id, username, rol FROM userweb WHERE username LIKE ?", ('%' + username + '%',))
    else:
...
def api_userweb_update(id):
    ...
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE userweb SET rol=? WHERE id=?", (rol, id))
    conn.commit()
    cursor.close()
    conn.close()
